...
const QStringList GeneralTaskUtility::splitAtComma(const QString &line) {
    ...
    for (int i = 0; i < line.size(); ++i) {
        if (line[i] == '\\') ++backslashCounter;
            ...
            if (i >= 2 && line[i] == ',' && line[i-1] == ',' && line[i-2] == '\\') continue;
...
const QString GeneralTaskUtility::removeBackslashBeforeComma(const QString &line) {
    ...
    for (int i = 0; i < string.size() - 1; ++i) {
        if (string[i] == '\\' && string[i+1] == ',') string.remove(i, 1);
    }
    return string;
}
...
const QString GeneralTaskUtility::removeComment(const QString &line) {
    ...
    for (int i = 0; i < string.size(); ++i) {
        if (string[i] == '\\') ++backslashCounter;
        else if (string[i] == '#') {
            if (backslashCounter % 2 == 0) return string.remove(i, string.size()-i);
            else string.remove(i-1,1);
            backslashCounter = 0;
        }
        else backslashCounter = 0;
    }
