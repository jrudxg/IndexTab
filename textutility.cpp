...
const std::function<bool(QString, QVariantMap&)> TextUtility::mapP = [](QString line, QVariantMap &map) {
...
const std::function<bool(QString, QVariantMap&)> TextUtility::mapS = [](QString line, QVariantMap &map) {
...
const bool TextUtility::setText(const QString finalText, unsigned int endingSequence, QString imagePath,  QVariantMap &map) {
    if (endingSequence > getEndingSequences().size()) return false;
...
void TextUtility::generateText(QQmlComponent* textComponent, QQuickItem* scene, QVariantMap &map) {
    ...
    if (scale != QVariant()) QQmlProperty(text, "scale").write(scale);
    else {
