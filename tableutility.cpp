...
const std::function<bool(QString, QVariantMap&)> TableUtility::mapP = [](QString line, QVariantMap &map) {
...
const std::function<bool(QString, QVariantMap&)> TableUtility::mapR = [](QString line, QVariantMap &map) {
...
const void TableUtility::addRowsToValuesAndDictionary(QVector<QString>* requiredValues, QMultiMap<QString, std::function<bool(QString, QVariantMap&)>>* dictionary, const int rowCount) {
...
const int TableUtility::generateTableModel(QString line, QVariantMap &map) {
    ...
    if (potentialSize.size() != 2) return 0;
    ...
    for (int i = 1; i < values.size(); ++i) {
        columnHeaders << values[i];
    }

    if (columnHeaders.size() > size.columnCount) return 0;
    TableModel* model = new TableModel(size.rowCount, size.columnCount, QStringList(), columnHeaders );
